"use client"
import { Fragment, useRef, FormEvent, useEffect } from 'react';
...
function Modal() {
  ...
  const [isOpen, closeModal, isUpdate, selectedTask] = useModalStore((state) => [
    state.isOpen,
    state.closeModal,
    state.isUpdate,
    state.selectedTask,
  ...
  const isUpdating = Boolean(selectedTask);

  useEffect(() => {
    if (isUpdating && selectedTask) {
      setNewTaskInput(selectedTask.title || '');
      setNewDesc(selectedTask.description || '');
      setNewTaskType(selectedTask.status || 'todo');
    } else {
  ...
  }, [isUpdating, selectedTask, setNewTaskInput, setNewDesc, setNewTaskType]);
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (isUpdating && selectedTask) {
      // Update Task
      updateTask(newTaskInput, newDesc, newTaskType,selectedTask, image);
        ...
        <div className="fixed inset-0 flex items-center justify-center">
          ...
          >
            <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all relative">
              ...
              >
                {isUpdating ? 'Update Task' : 'Add A Task'}
              </Dialog.Title>
              <div className="mt-2">
                <input
                  type="text"
